...
from acie.repo_id import resolve_repo_root
...
def _run(repo_path, agent, payload, *, db_path, registered):
    write_queue = WriteQueue(db_path_for=lambda repo_key: db_path)
    handle_notify_hook(
        agent=agent,
        repo_path=repo_path,
        payload=payload,
        register=lambda rp: registered.append(rp),
...
# -- decision 12: cold-start register() call --


def test_notify_hook_always_calls_register_first(tmp_path):
    repo = _git_repo(tmp_path)
    (repo / "mod.py").write_text("def foo():\n    pass\n")
    _commit(repo)
    registered = []

    _run(str(repo), "git", "", db_path=str(tmp_path / "index.sqlite"), registered=registered)
...
def test_git_agent_with_no_prior_head_sha_just_records_current_head_and_reindexes_nothing(tmp_path):
    ...
    _run(str(repo), "git", "", db_path=db_path, registered=[])
...
def test_git_agent_diffs_since_last_recorded_head_and_reindexes_changed_files(tmp_path):
    ...
    _commit(repo, "first")
    db_path = str(tmp_path / "index.sqlite")
    _run(str(repo), "git", "", db_path=db_path, registered=[])  # records the starting HEAD
...
def test_git_agent_is_a_no_op_when_head_has_not_moved(tmp_path):
    ...
    _run(str(repo), "git", "", db_path=db_path, registered=[])  # same HEAD, called again
...
def test_claude_code_agent_reindexes_the_file_named_in_tool_input(tmp_path):
    ...
    _run(str(repo), "claude-code", payload, db_path=db_path, registered=[])
...
def test_claude_code_agent_with_malformed_payload_is_a_no_op(tmp_path):
    ...
    _run(str(repo), "claude-code", "not json", db_path=db_path, registered=[])  # must not raise
...
def test_claude_code_agent_ignores_a_file_outside_the_repo(tmp_path):
    ...
    _run(str(repo), "claude-code", payload, db_path=db_path, registered=[])  # must not raise
...
def test_codex_agent_parses_file_paths_out_of_a_unified_diff_command(tmp_path):
    ...
    _run(str(repo), "codex", payload, db_path=db_path, registered=[])
...
def test_codex_agent_with_no_command_field_is_a_no_op(tmp_path):
    ...
    _run(str(repo), "codex", payload, db_path=db_path, registered=[])  # must not raise
...
def test_unrecognized_agent_name_is_a_silent_no_op(tmp_path):
    ...
    _run(str(repo), "some-future-agent", "{}", db_path=db_path, registered=[])  # must not raise
