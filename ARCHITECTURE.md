...
- **Exactly one ACIE daemon runs per computer.** It owns a single write queue and all `.acie`/`~/.acie` on-disk state. Every agent-spawned MCP server process is a client of this one daemon, never a direct writer to the SQLite files. This mirrors the daemon pattern already used by the separate, unrelated SALTMDB project on this machine — cited as prior art/precedent only, not a dependency.
...
**Query-time exposure**: the terse/default response mode from MCP tools hides confidence and provenance fields entirely. Passing `full=true` on a query reveals confidence/provenance per returned item. An optional `min_confidence` parameter can narrow results server-side. Default filtering behavior is unrestricted — i.e. `AMBIGUOUS` facts are not silently excluded by default. For confidence-sensitive aggregate counting (e.g. blast-radius / impact analysis), rather than silently filtering by confidence, ACIE instead reports counts broken out by confidence tier (e.g. "12 EXTRACTED, 3 AMBIGUOUS affected symbols" rather than a single blended number).
...
1. **Filesystem watcher (the backbone)** — a daemon-managed OS-level filesystem watcher (inotify on Linux / FSEvents on macOS / ReadDirectoryChangesW on Windows, e.g. via Python's `watchdog` library) watches each actively-indexed repo's tree, respecting the repo's ignore rules, with debouncing/coalescing of rapid successive events. This catches every change regardless of what tool produced it, with zero cooperation required from any editor/agent/tool — it is the mechanism that makes ACIE work correctly even with no other integration installed at all.
2. **Git hooks** (`post-commit`, `post-merge`, `post-checkout`, `post-rewrite`) — give the daemon an exact old-SHA/new-SHA diff as an efficiency hint for bulk changes (e.g. a branch checkout that touches hundreds of files at once), so the daemon can process one targeted diff instead of a flood of many individual filesystem-watcher events.
...
4. **Lazy staleness check at query time** — the final safety-net fallback: if a query touches a file whose on-disk state doesn't match what's indexed (and none of tiers 1–3 caught it), the daemon detects and corrects this at query time.
...
Exact staleness-check mechanics (e.g. whether staleness is detected via file modification-time or a content hash) and the exact filesystem-watcher debounce/coalescing window size are listed under "Not Yet Specified" below.
...
ACIE's v0 MCP server exposes 8 tools total (a locked 7-tool MVP surface, plus 1 additional `explain` tool added afterward to cover observation-history retrieval). Every tool response uses a shared response envelope: `{index_generation, results, total_count, truncated, next_cursor}`. `next_cursor` is an opaque string the caller round-trips as the `cursor` input to fetch the next page; it is `null`/absent once `truncated` is `false`. Passing `full=true` on any tool reveals confidence/provenance fields per result item (see "Provenance & Confidence Semantics"); the default (terse) response omits them.
...
| Tool | Notes |
|---|---|
| `find_symbol` | `name` is a required substring match; `kind` is an optional enum filter; `path_glob` optionally scopes the search to a subset of files. |
| `get_definition` | `symbol_id` or a file position — mutually exclusive. |
| `find_references` | Same mutually-exclusive shape as `get_definition`. |
| `list_imports` | `file` required. Import edges are always `EXTRACTED` confidence (tree-sitter parses imports completely unambiguously), so `full=true` is effectively a no-op here. |
...
- Monorepo, multi-language-in-one-repo, generated-file, git-worktree, symlink, and temporarily-broken-repo handling. Explicitly deferred; the likely v0 stance is "single-language, single-root repos only" but this is not locked.
...
- Exact staleness-check mechanics (file modification-time vs. content hash) and the filesystem-watcher debounce/coalescing window size.
...
- CLI UX surface beyond the core daemon/index primitives (e.g. an `acie query` command, an `acie serve-mcp` command), and specifically an automated `acie install-hooks` merge-installer (intentionally deferred pending real v0 dogfeeding usage data, per the Agent Hook Integration decision above).
