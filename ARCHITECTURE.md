...
4. **Lazy staleness check at query time** — the final safety-net fallback: if a query touches a file whose on-disk state doesn't match what's indexed (and none of tiers 1–3 caught it), the daemon detects and corrects this at query time. Not yet implemented — tiers 1-3 above cover the cases dogfooding has exercised so far; see "Not Yet Specified".
...
- Monorepo, multi-language-in-one-repo, generated-file, git-worktree, symlink, and temporarily-broken-repo handling. Explicitly deferred; the likely v0 stance is "single-language, single-root repos only" but this is not locked. (Note: the `WriteQueue`/`BootstrapCoordinator`/watcher registries key their in-memory state on the raw `repo_path` string rather than the canonical worktree-collapsing `resolve_repo_id`, so two worktrees of one repo currently get separate in-memory state despite sharing one on-disk `index.sqlite` — a known pre-existing inconsistency, not yet fixed.)
...
- Tier 4 of "Incremental Indexing" (lazy staleness check at query time) — tiers 1-3 are implemented; tier 4 is not yet built.
