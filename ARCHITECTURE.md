...
**v1 capability C (architecture-level queries, wayfinder ticket 47d8cd0d) — in progress.** Slice C1 lands the foundational piece the eventual `architecture(root, granularity, ...)` MCP tool (C2) will consume, not the tool itself (no 10th tool is exposed yet): `src/acie/module_paths.py` extracts indexer.py's originally-private file-path↔dotted-module-name derivation (`path_to_dotted`/`module_path_matches`, formerly `_module_path_matches`) into shared top-level code, since `acie.tools.architecture` needs the identical suffix-tolerant derivation and indexer.py/`acie.tools` are dependency-graph siblings (same "wait for a real 2nd caller" extraction pattern as B2's `pytest_conventions.py`). `src/acie/tools/architecture.py`'s `build_dotted_name_index(symbol_store)` then builds a one-time `dotted_name -> file_path(s)` index over every live `kind='module'` symbol, keyed by every dotted suffix of each module's derivation — this is what will let C2 classify an `imports` edge's raw dotted-name target (edges keep this as a string forever, see "Canonical IR / Data Model") as internal (resolves to a real file) or external, without ACIE ever needing PYTHONPATH/source-root configuration. This closes a design risk flagged during the ticket's own grilling session (a src-layout repo's `src/` prefix silently causing every internal import to misclassify as external): the derivation was already suffix-tolerant, not an exact full-path match, so no fix was needed — empirically verified in `tests/test_module_paths.py`/`tests/tools/test_architecture.py`'s dedicated src-layout cases, not just re-read and assumed correct.
