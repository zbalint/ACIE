...
| `acie notify-hook --agent <name>` | Wiring is out of this document's scope — its contract (stdin payload, silent no-op on an unreachable daemon, ~200ms fire-and-forget timeout) is already fully locked in `ARCHITECTURE.md`'s "Agent Hook Integration" section. |
...
- **Watcher / git-hooks / agent-hooks wiring** — tiers 1–3 of `ARCHITECTURE.md`'s incremental-indexing precedence. Only bootstrap indexing and tier-4 lazy-staleness (this document's "Bootstrap Indexing & `INDEX_NOT_READY`") are this phase's implementation boundary.
