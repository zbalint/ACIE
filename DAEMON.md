...
**Resolved during implementation** (`src/acie/daemon/lsp_enrichment.py`): `run_enrichment_pass()` is D3's bounded, opportunistic consumer of that client. It opens each candidate file once, asks pyright serial `textDocument/definition` requests only for currently `AMBIGUOUS` or unpersisted unresolved `calls`/`inherits` sites, accepts only one in-repository `Location` or `LocationLink` target that maps to a live ACIE symbol, and submits each resulting `INFERRED` relation through the per-repo `WriteQueue`. Provenance comes from that pass's live LSP `serverInfo`; missing capability/process and per-site failures remain no-ops. Trigger wiring is implemented in `src/acie/daemon/runtime.py` as an internal daemon-lifecycle side effect.

**Resolved during implementation** (`src/acie/daemon/bootstrap.py`, `src/acie/daemon/runtime.py`): `BootstrapCoordinator` invokes its `on_indexed` hook after each non-empty bootstrap or one-time cross-file migration catch-up. The daemon owns one long-lived `PyrightProcessRegistry` shared by all triggered repos, and always hands the enrichment pass to a fresh thread because running it inline would deadlock the bootstrap writer thread on a same-repo write-queue Future. The D6 trigger is one-shot per repo per daemon process lifetime. The existing bootstrap readiness/migration-marker fire-and-forget race remains accepted because its worst case is a harmless duplicate pass, not a deadlock or corruption.

**Resolved during implementation** (Capability E, Slice E1, `src/acie/daemon/bootstrap.py`, `src/acie/daemon/enrichment_scheduler.py`, `src/acie/daemon/runtime.py`, `src/acie/daemon/watcher.py`): every bootstrap, migration, startup-reconciliation, and live-watcher trigger routes through one daemon-owned `EnrichmentScheduler`. Its lock-protected per-repo running/pending state turns the accepted bootstrap double-fire plus reconciliation interleaving into at most one follow-up and never concurrent enrichment passes. Startup reconciliation compares a cheap git-native fingerprint (HEAD, `git diff HEAD`, and sorted untracked path/mtime/size entries) with `IndexMetaStore.last_enrichment_fingerprint`; a mismatch or unavailable fingerprint triggers a pass, and each successful pass records a fresh fingerprint. Watcher batches retain a 30-second quiet-period debounce with a 300-second deadline cap. Full reasoning and the regression coverage are in [`E1-DAEMON-RECURRING-TRIGGER-SPEC.md`](./E1-DAEMON-RECURRING-TRIGGER-SPEC.md).
