...
**Resolved during implementation** (`src/acie/daemon/lsp_enrichment.py`): `run_enrichment_pass()` is D3's bounded, opportunistic consumer of that client. It opens each candidate file once, asks pyright serial `textDocument/definition` requests only for currently `AMBIGUOUS` or unpersisted unresolved `calls`/`inherits` sites, accepts only one in-repository `Location` or `LocationLink` target that maps to a live ACIE symbol, and submits each resulting `INFERRED` relation through the per-repo `WriteQueue`. Provenance comes from that pass's live LSP `serverInfo`; missing capability/process and per-site failures remain no-ops. Trigger wiring and stale-candidate cleanup remain D6 and D4 work respectively.
