...
The daemon learns which worktree (and therefore which worktree-specific `index.sqlite`) a session's tool calls target from **ambient `cwd`**, not an explicit config flag.

`acie serve-mcp` captures `os.path.realpath(os.getcwd())` once at process startup — one MCP-server process serves exactly one agent session end-to-end, so this is safe as a per-process value, never shared or mutated across sessions. It forwards that canonical path as `repo_path`; the daemon resolves it through `repo_id.py` / `resolve_worktree_id` to the primary `repo_id` or the linked worktree's distinct `worktree_id`, and uses the corresponding `repo_root` and nested index path for dispatch.
...
When the daemon registers a worktree it has not seen before (no worktree-specific `index.sqlite`, or an incomplete prior bootstrap), it starts a bootstrap pass in the background — using the ready primary worktree's index as a seed and reindexing only changed paths when possible, otherwise falling back to a full walk — via that worktree's write queue rather than blocking the caller.

Every tool call touching that worktree returns the already-named `INDEX_NOT_READY` structured error until that worktree's bootstrap pass completes; the calling agent is expected to retry. A blocking first call would leave `INDEX_NOT_READY` a dead code path with nothing that could ever trigger it, and a multi-second blocking call is a worse agent-facing experience than a clear, retryable "not ready yet" signal.

**Resolved during implementation** (Slice 4, `src/acie/daemon/bootstrap.py`): `BootstrapCoordinator` is the real `repo_ready` implementation behind dispatch.py's seam. Readiness is a daemon-process-lifetime concept keyed by `worktree_id`; a primary worktree runs the normal full walk, while a linked worktree seeds from a ready primary and reindexes its changed paths, falling back to a full walk if the primary is unavailable. Registration submits one write-queue job per file and marks that worktree ready after its pass completes. This preserves the in-flight race fix: opening a worktree's write-queue connection creates its `index.sqlite` on disk before the first write commits, so `repo_ready()` trusts the in-memory `_in_progress` flag over mere file existence for any worktree this coordinator has started.

## Write-Queue Concurrency

**Per-worktree write-queue isolation.** The daemon runs one dedicated writer thread — not an asyncio task — plus one FIFO queue, *per indexed worktree*, not a single global thread/queue for the whole daemon. Each thread owns its own exclusive SQLite connection to that worktree's `index.sqlite`, opened once at thread creation. This matches the storage layer's existing per-worktree index isolation (`ARCHITECTURE.md`'s on-disk layout) and prevents one worktree's heavy reindex from starving another worktree's queries — with per-worktree isolation, an unrelated worktree's queries never wait behind another worktree's write queue at all.

Request-handling threads never touch a SQLite connection directly: they enqueue a transaction closure onto the target worktree's queue and block on a `concurrent.futures.Future` for the result. This is safe because incoming tool-call dispatch genuinely runs across multiple threads — confirmed against the installed `mcp==2.1.1` package, whose synchronous tool-handler path uses `anyio.to_thread.run_sync` (worker threads, not the event loop) — exactly the situation a dedicated writer thread + queue is designed to serialize.

`index_file()` (`src/acie/indexer.py`) is already the natural per-file unit of write work: bootstrap indexing already produces one write-queue item per file, not one per whole-worktree job, so no batching change is needed to make this per-worktree model work.
...
- **Creation**: lazy, on-demand. A worktree's first submitted write spins up that worktree's writer thread and queue; nothing is pre-created for worktrees the daemon has not seen a write for yet.
- **Teardown**: none. A worktree's thread lives for the daemon process's entire life, even if that worktree goes idle forever.
- **Cap**: none. No limit on the number of simultaneously-open per-worktree threads or per-worktree queue depth.

> **Shortcut** (per this project's deliberate-shortcut convention): unbounded per-worktree threads with no idle teardown is acceptable while a daemon's lifetime worktree count stays in the tens. Upgrade trigger: observed thread/resource pressure from accumulated per-worktree threads on a long-lived daemon — at which point add idle-timeout teardown (respawn lazily on the next write for that worktree) rather than redesigning the isolation model itself.

**Live-diagnosed follow-up fix (2026-09-04, SALTMDB memory `c90f7a6e`):** WAL mode, not a write-queue redesign. The per-worktree single-writer-thread model above was never the bug: it correctly serializes writers against each other. What was missing is reader/writer isolation at the SQLite layer itself. Every connection previously ran SQLite's default rollback-journal mode, under which a writer's commit takes an exclusive lock that blocks *every reader* — including `dispatch.py`'s fresh-per-call read connections — for the commit's entire duration, however long that turns out to be (live-traced on the running daemon to a multi-minute WSL2 ext4 journal-commit stall, `ps -T -p <pid> -o tid,stat,wchan` showing `jbd2_log_wait_commit`). Every raw `sqlite3.connect()` in the storage layer (the 4 store classes' `db_path`-only fallback, plus `write_queue.py`'s `_RepoWriter._run`) now goes through a shared `acie.storage.connection.open_connection()`, which sets `journal_mode=WAL` + `synchronous=NORMAL`. Readers now see a stable snapshot and are never blocked by an in-progress writer commit, regardless of how slow that commit's own fsync is. `open_connection()` retries the journal-mode transition briefly (only ever needed once per file, ever — WAL is sticky) since a concurrent connection holding an open write transaction at that exact moment makes the one-time transition fail with `SQLITE_LOCKED`, a failure mode that (unlike an ordinary read/write) does not respect sqlite3's own busy-timeout retry loop.

SALTMDB's dual foreground/background priority-lane split within one queue remains available to adopt later if ACIE ever needs finer-grained fairness *within a single worktree's own queue*; v0 ships with a single FIFO lane per worktree.
...
- **`WatcherRegistry`** is the tier-1 counterpart to `BootstrapCoordinator`/`WriteQueue` above — same shape (lazy per-worktree creation, no idle-teardown, lives for the daemon's whole process life) and the exact same trigger point: `runtime.py`'s `register_repo()` starts a worktree's `RepoWatcher` in the same call that starts its bootstrap pass, the first time any RPC (or `notify_hook` call) touches that `repo_path`.
- Each `RepoWatcher` wraps one `watchdog.observers.Observer` scoped to one canonical worktree root, debounces/coalesces raw filesystem events per-path over a ~500ms window (`threading.Timer`-based — `watchdog` has no built-in debounce), then submits one job to that worktree_id's write queue per touched path (`make_reindex_job`, promoted to a public name 2026-09-02 once tier 4 became its second caller — see below). That job does the hybrid mtime-then-hash staleness check (backed by a new `file_state` table, `src/acie/storage/file_state_store.py`) and calls the same `index_file()` every other reindex path uses — a delete is `index_file(path, "")` plus an explicit cleanup of the always-present module-kind symbol `index_file` alone doesn't remove (see the corrective memory linked from the grilling decision), and a rename is exactly that delete decomposed alongside a normal index of the new path.
...
**Follow-up fix** (SALTMDB f4bdfc9d decision 10, grilled and shipped 2026-09-02): `runtime.py`'s `dispatch()` now resolves a raw `repo_path` to its canonical `(worktree_id, repo_root)` pair once per request (`resolve_worktree_id`/`resolve_repo_root`, `repo_id.py`) and threads both through — `WriteQueue`/`BootstrapCoordinator` key their in-memory state on `worktree_id`; `db_path_for` delegates the ID-to-directory decision to `repo_id.py` (the primary keeps `~/.acie/repos/<repo-id>/index.sqlite`, linked worktrees use nested `worktrees/<worktree-id>/index.sqlite`), `WatcherRegistry` keys on `repo_root` (the actual directory an `Observer` watches — genuinely different worktree directories get separate watchers, but two spellings of the same worktree no longer get a duplicate one), and `BootstrapCoordinator.register`/its injected `walk_repo` take both the worktree_id for readiness bookkeeping and repo_root for the actual walk. `handle_notify_hook` receives the resolved worktree_id and repo_root directly rather than resolving or registering them itself.
...
`extract_staleness_target(method, params)` (`staleness.py`) is a pure function returning the one repo-relative `.py` path a request's params name, or `None`. `dispatch()` calls the module-level `ensure_fresh(write_queue, worktree_id, repo_root, method, params, timeout=2.0)` (`runtime.py`) after `register_repo()` but only once `bootstrap.repo_ready(worktree_id)` is already `True` (no point queueing staleness work ahead of a first bootstrap that will index the file anyway). `ensure_fresh` is a plain module-level function, not a `create_daemon()` closure, specifically so it stays unit-testable against a real `WriteQueue` without a full daemon/socket harness.

When `extract_staleness_target` returns a path, `ensure_fresh` submits `make_reindex_job(repo_root, rel_path)` — the exact same job tier 1's watcher already uses, promoted from `_make_watch_job` to a public name once tier 4 became its second caller (reuse, not a parallel staleness-check implementation) — to that worktree's write queue and blocks on the Future for up to 2 seconds. On success the query proceeds against a guaranteed-fresh index for that one file; on timeout or any exception, it logs a warning and proceeds anyway with whatever the index currently has — this must never delay or fail a query beyond its bounded budget, the same "never break the caller" principle `notify-hook`'s own 200ms fire-and-forget timeout already uses (see `ARCHITECTURE.md`'s "Agent Hook Integration"). A redundant reindex racing an in-flight watcher job for the same edit is harmless and cheap (the mtime check makes a no-op reindex nearly free), matching tier 1/3's already-shipped "no cross-tier dedup" decision.

## LSP/pyright Enrichment Subprocess Lifecycle

**Resolved during implementation** (`src/acie/daemon/pyright_process.py`): `PyrightProcessRegistry` owns the standalone, opportunistic lifecycle of one `pyright-langserver --stdio`-equivalent child per canonical `repo_root`. `basedpyright` is a core ACIE dependency (not an optional extra), so binary discovery checks `basedpyright-langserver` first, falling back to `pyright-langserver` for a pre-existing plain-`pyright` install on `PATH`; it captures the process's version only through a best-effort probe, exposes injected discovery/spawn seams for tests, restarts idle deadlines on use, and transparently respawns a child found dead on a later `ensure_process()` call. Its 15-minute idle timeout reclaims pyright's potentially large warmed-process memory footprint; its shutdown path cancels every timer before bounded terminate/wait/kill calls share one overall deadline. A spawn failure or crash still returns/respawns as `None`/fresh rather than raising — the dependency being mandatory now only guarantees the binary is *installed*, not that it will always successfully spawn — preserving the deterministic tree-sitter baseline's correctness independence without a runtime, dispatch, or MCP-tool integration in this slice.
...
**Resolved during implementation** (`src/acie/daemon/lsp_enrichment.py`): `run_enrichment_pass()` is D3's bounded, opportunistic consumer of that client. It opens each candidate file once, asks pyright serial `textDocument/definition` requests only for currently `AMBIGUOUS` or unpersisted unresolved `calls`/`inherits` sites, accepts only one in-repository `Location` or `LocationLink` target that maps to a live ACIE symbol, and submits each resulting `INFERRED` relation through the worktree's `WriteQueue`. Provenance comes from that pass's live LSP `serverInfo`; missing capability/process and per-site failures remain no-ops. Trigger wiring is implemented in `src/acie/daemon/runtime.py` as an internal daemon-lifecycle side effect.

**Resolved during implementation** (`src/acie/daemon/bootstrap.py`, `src/acie/daemon/runtime.py`): `BootstrapCoordinator` invokes its `on_indexed` hook after each non-empty bootstrap or one-time cross-file migration catch-up. The daemon owns one long-lived `PyrightProcessRegistry` shared by all triggered worktrees, and always hands the enrichment pass to a fresh thread because running it inline would deadlock the bootstrap writer thread on a same-worktree write-queue Future. The D6 trigger is one-shot per worktree per daemon process lifetime. The existing bootstrap readiness/migration-marker fire-and-forget race remains accepted because its worst case is a harmless duplicate pass, not a deadlock or corruption.

**Resolved during implementation** (Capability E, Slice E1, `src/acie/daemon/bootstrap.py`, `src/acie/daemon/enrichment_scheduler.py`, `src/acie/daemon/runtime.py`, `src/acie/daemon/watcher.py`): every bootstrap, migration, startup-reconciliation, and live-watcher trigger routes through one daemon-owned `EnrichmentScheduler`. Its lock-protected per-worktree running/pending state turns the accepted bootstrap double-fire plus reconciliation interleaving into at most one follow-up and never concurrent enrichment passes. Startup reconciliation compares a cheap git-native fingerprint (HEAD, `git diff HEAD`, and sorted untracked path/mtime/size entries) with `IndexMetaStore.last_enrichment_fingerprint`; a mismatch or unavailable fingerprint triggers a pass, and each successful pass records a fresh fingerprint. Watcher batches retain a 30-second quiet-period debounce with a 300-second deadline cap. Full reasoning and the regression coverage are in [`E1-DAEMON-RECURRING-TRIGGER-SPEC.md`](./E1-DAEMON-RECURRING-TRIGGER-SPEC.md).

**Resolved during implementation** (`src/acie/daemon/merge_policy.py`): `apply_enrichment_write()` enforces D4's write-time merge rule inside the queue job, refusing an enrichment write that would regress a more-certain existing relation such as `EXTRACTED`, then writing the winner before retiring same-source, same-site, different-target `AMBIGUOUS` sibling rows. Cross-pass `INFERRED` reconciliation and transaction wrapping across the write and retire calls remain outside this slice; daemon trigger cadence is now resolved by E1's shared scheduler and recurring watcher/reconciliation triggers.
...
- On receiving `shutdown`, the daemon: stops accepting new RPCs (returning `DAEMON_SHUTTING_DOWN` to anything that arrives after this point), drains every per-worktree write queue to completion, deletes `~/.acie/daemon.json`, sends the RPC's success response, then forces the process to exit outright.
...
- **Multi-repo behavior validation** — the design above accounts for multiple worktrees and repositories structurally throughout (per-worktree write queues, per-worktree store instances, per-worktree readiness flags); actually exercising it under load is deferred to the implementation phase.
