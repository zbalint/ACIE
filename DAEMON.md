...
**Resolved during implementation** (`src/acie/daemon/bootstrap.py`, `src/acie/daemon/runtime.py`): `BootstrapCoordinator` invokes its `on_indexed` hook after each non-empty bootstrap or one-time cross-file migration catch-up. The daemon owns one long-lived `PyrightProcessRegistry` shared by all triggered repos, and always hands the enrichment pass to a fresh thread because running it inline would deadlock the bootstrap writer thread on a same-repo write-queue Future. The trigger is one-shot per repo per daemon process lifetime; watcher edits do not re-enrich the whole repository. The `87e7e80b` staleness-window follow-up is resolved without a per-site freshness guard in v1; the full reasoning and named upgrade trigger are recorded in `D6-DAEMON-TRIGGER-SPEC.md`. A narrow, accepted race remains between the from-scratch bootstrap path's synchronous readiness flip and its own fire-and-forget migration-flag write, which could in theory double-fire the trigger for one repo; named `# shortcut:` at its call site in `bootstrap.py`'s `on_bootstrap_done` (worst case is a harmless duplicate enrichment pass, not a deadlock or corruption).

**Resolved during implementation** (`src/acie/daemon/merge_policy.py`): `apply_enrichment_write()` enforces D4's write-time merge rule inside the queue job, refusing an enrichment write that would regress a more-certain existing relation such as `EXTRACTED`, then writing the winner before retiring same-source, same-site, different-target `AMBIGUOUS` sibling rows. Cross-pass `INFERRED` reconciliation and transaction wrapping across the write and retire calls remain outside this slice; daemon trigger cadence is now resolved by D6's one-shot bootstrap/migration-catch-up trigger.
